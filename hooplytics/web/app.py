...
def _render_slate_brief_panel(roster: dict, api_key: str) -> None:
    """Render the Daily Slate Brief card on the home page.

    Generation is gated behind an explicit user action — the brief never
    auto-runs just because an AI key is present, since each render burns
    one API credit. We surface a CTA button on cold load, and only generate
    after the user clicks. Once generated, the result is held in
    ``st.session_state`` so subsequent reruns within the same session don't
    re-fire the call.
    """
    provider = _active_ai_provider()
    provider_label = PROVIDER_LABELS.get(provider, "AI")
    ...
    # Session-scoped storage so a brief survives reruns but never auto-fires
    # on a fresh page load. Keyed on today + roster so a roster change or
    # date rollover invalidates the displayed brief.
    state_key = f"slate_brief_v2|{today}|{_roster_key()}|{provider}|{selected_model}"
    requested_key = f"{state_key}|requested"

    st.markdown(
        '<div class="hl-card" style="margin-top:1.25rem;">'
        '<p class="hl-card-title">Daily Slate Brief</p>'
        f'<p class="hl-subtle" style="margin-bottom:0.5rem;">An AI-written read on '
        f'tonight\'s slate, generated on demand by {provider_label} ({selected_model}). '
        f'Each generation uses one API call.</p>',
        unsafe_allow_html=True,
    )

    cached_brief = st.session_state.get(state_key)
    if cached_brief:
        # Already generated this session — show it with a Refresh affordance.
        cols = st.columns([6, 1])
        cols[0].markdown(
            f'<p style="font-size:0.95rem;line-height:1.5;margin:0;">{cached_brief}</p>',
            unsafe_allow_html=True,
        )
        if cols[1].button("Refresh", key=f"slate_brief_refresh_btn|{today}",
    ...
    if not st.session_state.get(requested_key):
        # No brief yet, no pending request — show the CTA and stop.
        if st.button(
            "Generate brief",
            key=f"slate_brief_generate_btn|{today}",
            type="primary",
            help=f"Calls {provider_label} once and writes a slate-level read.",
        ):
            st.session_state[requested_key] = True
            st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    # User asked for a brief on a previous run — generate it now.
    ...
    if brief:
        ...
        st.caption("No brief produced — try the Generate button again.")
    st.markdown('</div>', unsafe_allow_html=True)
...
def page_home(roster: dict, api_key: str) -> None:
    ...
    # Last night's NBA results (Odds API /scores) — keep this near the top
    # so the dashboard always opens with last night's box-score recap, even
    # for users with an AI key configured.
    _render_recent_scores_panel(api_key)

    # Daily AI Slate Brief — its own section below last night's recap.
    # Only renders when AI is configured AND the user explicitly clicks
    # "Generate brief" inside the panel.
...
# Per-player diagnostic strings the PDF builder uses to choose the right
# placeholder text when a player has zero resolved historical lines. Keeping
# the values short + namespaced so the values stay easy to grep.
_HISTORY_STATUS_HAS_LINES = "has_lines"
_HISTORY_STATUS_NO_API_COVERAGE = "no_api_coverage"
_HISTORY_STATUS_NO_GAME_OVERLAP = "no_game_overlap"
_HISTORY_STATUS_EMPTY_CACHE = "empty_cache"


@st.cache_data(show_spinner=False, ttl=60 * 30, max_entries=4)
def _player_history_lines_vs_outcomes(
    roster_key: str, last_n: int = 10,
    ...
    Push. Returns ``(history_dict, status_dict)``:

    * ``history_dict``: ``{player: DataFrame}`` with the most recent
      ``last_n`` resolved line/outcome rows per player. Players with no
      joinable history are omitted (the PDF renderer falls back to a
      placeholder for them).
    * ``status_dict``: ``{player: str}`` with one of:

      - ``"has_lines"`` — resolved lines computed; table will populate.
      - ``"no_api_coverage"`` — Odds API has zero rows for this player in
        the cached window (the placeholder explains the bookmaker simply
        hasn't posted props for them).
      - ``"no_game_overlap"`` — Odds API does have rows for the player but
        none align with their modeling-frame game dates (placeholder
        suggests refreshing the player's game log / running the backfill).
      - ``"empty_cache"`` — the historical odds cache is empty altogether.
    """
    try:
    ...
    if odds_df.empty:
        return {}, {p: _HISTORY_STATUS_EMPTY_CACHE for p in roster}

    modeling_df = _modeling_frame(roster_key)
    if modeling_df.empty or "player" not in modeling_df.columns:
        # Treat as game-log gap so the placeholder points at the game-log
        # refresh path rather than blaming the bookmaker.
        return {}, {p: _HISTORY_STATUS_NO_GAME_OVERLAP for p in roster}
    ...
    # Canonicalize via the shared odds-module helper so suffix and
    # punctuation handling stays consistent with the live-line ingest path
    # — without this, "Jabari Smith Jr." (NBA Stats) and "Jabari Smith"
    # (Odds API) hash to different keys and the join silently drops rows.
    from hooplytics.odds import _canon_name as _canon
    ...
    for canon_name, roster_name in canon_to_roster.items():
        ...
        if sub_odds.empty:
            # Nothing posted by the bookmaker for this player at all in the
            # cached window. Most common cause: the player's team isn't on
            # the active slate the API has been tracking (e.g. a non-playoff
            # team during the postseason).
            status[roster_name] = _HISTORY_STATUS_NO_API_COVERAGE
            continue

        sub_games = games[games["_canon"] == canon_name]
        if sub_games.empty:
            status[roster_name] = _HISTORY_STATUS_NO_GAME_OVERLAP
            continue
        ...
        if not rows:
            # Lines exist for this player and they have a game log, but the
            # two never line up by date. Almost always a stale game log
            # (the cache hasn't ingested recent playoff games yet).
            status[roster_name] = _HISTORY_STATUS_NO_GAME_OVERLAP
            continue
        df = pd.DataFrame(rows).sort_values("game_date", ascending=False).head(last_n)
        history[roster_name] = df.reset_index(drop=True)
        status[roster_name] = _HISTORY_STATUS_HAS_LINES
...
from hooplytics.odds import _canon_name as _canon_player_name
...
def _render_projections_report(
    ...
    # ── Historical odds backfill (off by default — guards Odds API credits) ─
    # The historical Lines-vs-Outcomes table inside the PDF can populate from
    # whatever is already in the local cache (live snapshots from previous
    # days, or manual ingests via `hooplytics ingest-odds`). When this toggle
    # is on, we top up the cache from the Odds API historical endpoint —
    ...
    # Historical odds backfill is OPT-IN to protect Odds API credits — the
    # historical endpoint can burn thousands of credits on a cold cache. When
    ...
    if api_key and backfill_enabled and not _is_demo_mode():
        ...
        with st.spinner(f"Backfilling {days_requested} day(s) of historical odds…"):
            try:
                fetched = _backfill_recent_historical_odds(api_key, days=days_requested)
                if fetched:
                    # ``load_cached_historical_odds`` is a plain function with
                    # its own fingerprint-based memo — it auto-invalidates on
                    # the next call once the new JSON files land on disk. The
                    # report-side join cache IS Streamlit-decorated, so bust
                    # that one explicitly.
                    _player_history_lines_vs_outcomes.clear()
                    st.caption(f"Backfilled {fetched} new date(s) of historical odds.")
                else:
                    st.caption("Historical odds cache already covers the requested window.")
            except Exception as exc:
                st.warning(f"Historical odds backfill skipped: {exc}")
    ...
    matchup_preds = _build_slate_predictions(
        ...
        # Scope to games where the user actually has rostered players —
        # otherwise the section listed every NBA game on the slate, most of
        # which are irrelevant to the user's roster.
        roster_only=True,
    )
    if matchup_preds:
    ...
    history_dict, history_status = _player_history_lines_vs_outcomes(_roster_key())
    try:
        pdf_bytes = build_pdf_report(
            ...
            player_history=history_dict or None,
            player_history_status=history_status or None,
